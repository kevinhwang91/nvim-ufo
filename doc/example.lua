...
local another_handler = function(_, lnum, endLnum, width, truncate, ctx)
    local getFoldVirtText = ctx.get_fold_virt_text
    local virtText = getFoldVirtText(lnum)
    local minWidth = 25
    local offset = 0
    ...
    while virtTextWidth(virtText) < minWidth and offset <= maxOffset do
        ...
        virtText = concatVirtTexts(
            virtText,
            separator,
            compressSpaces(trimIndent(getFoldVirtText(lnum + offset)))
        )
    end
...
local function customizeFoldTextWithFunc()
    require('ufo').setup({
        enable_get_fold_virt_text = true,
        fold_virt_text_handler = another_handler,
    })
end
